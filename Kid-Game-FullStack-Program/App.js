...
document.getElementById("checkanswer").addEventListener("click", function () {
    const correctAnswers = FindCorrectAnswer(); // Obter respostas com base no nível
...
function TIMECOUNTDOWN() {
    ...
    interval = setInterval(() => {
        ...
        if (countdown <= 0) {
            ...
            disableinputfield();
            checkPressed = true; // Impede a entrada após o tempo esgotar
        }
...
// Automatically set the level to 'easy' and start the game when the page loads
window.onload = () => {
    setLevel('easy');  // Automatically set the level to 'easy' on page load
    console.log('Game started in Easy mode');
    AllBoxesFilled();  // Check if the button should be enabled based on existing input values
    console.log('Game started in Easy mode');
};
